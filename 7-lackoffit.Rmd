...
For our example, we'll work with data from 519 encounter histories of Cory's shearwaters (*Calonectris diomedea*) marked as adults between 2001 and 2008 at Pantaleu Islet (Balearic Archipelago, Spain). If you perform goodness-of-fit tests on this dataset, as in Section \@ref(gof), you will end up with a significant trap-dependence effect (as well as a transience effect). To build up our model and explicitely account for this lack of fit, we first define the states and observations:
...
Our estimates are very (very) similar to those obtained by @pradeltrapdep2012, check out their Table 1. Interestingly, when we fit the same model by ignoring trap-dependence, we get the following results: 
```{r echo = FALSE}
library(MCMCvis)
...
To relax the first-order assumption, we move to a second-order Markov description, and what happens at $t+1$ depends on the site at $t$ and at $t−1$. This idea, often called a memory model, was introduced in multisite capture–recapture by @hestbeck1991estimates and @BrownieEtAl1993, then formulated cleanly within the HMM framework by @rouan2009memory and @pradel_multievent_2005.
...
which we can compare to the results obtained by @pradel_multievent_2005 in his Table 1: 
...
ours <- tribble(
  ...
  mutate(Ours = sprintf("%.2f (%.2f–%.2f)", mean, lo, hi)) |>
  select(Transition, Condition, Ours)
...
## Accomodating individual heterogeneity
...
# ---- Posterior summaries from your fit ----
post <- data.frame(
...
# NOTE: If your model's 'pi' was defined as the proportion in *class 2*,
# set pi_truth <- 0.80 instead of 0.20.
pi_truth <- 0.20
truth <- c(
  phi = 0.70,   # survival
...
# and speed things up by pooling identical encounter histories with a weight "size".
...
hmm.phipmix <- nimbleCode({
  ...
  # -------------------------------------------------
  # Parameters:
  # pi: initial state probability C1   (mixture weight for class 1)
  # phi: survival probability          (shared across classes)
  # pp1: recapture probability C1      (detection for class 1)
  # pp2: recapture probability C2      (detection for class 2)
  # -------------------------------------------------
  # States (S):
  # 1 alive (A1)  -> class 1
  # 2 alive (A2)  -> class 2
...
# Try to read posterior summaries from your object; else use the numbers you posted
post <- if (exists("mcmc.phipmix.marginalized")) {
  S <- MCMCvis::MCMCsummary(mcmc.phipmix.marginalized, params = c("phi","pi","pp1","pp2"))
  data.frame(
    param = rownames(S),
    mean  = S[,"mean"],
    lcl   = S[,"2.5%"],
  ...
  data.frame(
    param = c("phi","pi","pp1","pp2"),
    mean  = c(0.72, 0.72, 0.27, 0.79),
    lcl   = c(0.69, 0.54, 0.18, 0.65),
    ucl   = c(0.75, 0.87, 0.35, 0.92)
  )
}

# Data-generating values, remapped to the model’s ordering constraint (pp1 < pp2)
# Sim truth was: phi=0.70, class1 proportion=0.20 (high p=0.80), class2 p=0.30.
# With pp1 < pp2, we compare to: pp1=0.30, pp2=0.80, and pi = proportion in lower-detectability class = 0.80.
truth <- c(phi = 0.70, pi = 0.80, pp1 = 0.30, pp2 = 0.80)
...
A question that remains is the number of classes we should use. In other words, why 2 classes and not 3 or 4? One option is to fit models with more classes and select among them (see @cubaynes2012). Alternatively, you can take a non-parametric route and let the data decide how many classes are needed; this is relatively easy in NIMBLE (see @turek_bayesian_2021). 
