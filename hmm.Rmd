...
The likelihood is the probability of the observed data $y$ given the parameters of our model, the unkowns. Here the unknows are the survival and detection probabilities $\phi$ and $p$, and the latent states $z$. Using the definition of a conditional probability, then the likelihood of a Markov chain, we get that the likelihood of an HMM for a single individual $i$:
...
To obtain the likelihood of the whole dataset, we need to multiply this individual likelihood for each animal $\displaystyle{\prod_{i=1}^N{\Pr(\mathbf{y}_i)}}$. When several individuals have the same contribution, calculating their individual contribution only once can greatly reduce the computational burden, as illustrated in Section \@ref(pooled-likelihood).
...
In some situations, you will not be interested in inferring the hidden states $z_{i,t}$, so why bother estimating them? The good news is that you can get rid of the states, so that the likelihood is a function of survival and detection probabilities $\phi$ and $p$ only. By the way, this is the function you would maximize in a Frequentist approach. 

Using the formula of total probability, we get by summing over all possible states:
...
This function is complex to evaluate because it involves many operations ($TN^T$ operations to be precise, where $T$ is the length of encounter histories and $N$ is the number of hidden states), which number increases exponentially as the number of states increases. This complexity precludes using this method to get rid of the states. Fortunately, we have another algorithm in the HMM toolbox that is useful.  

### Forward algorithm

Several products that are computed several times. What if we could store the results and use them later. Recurrence. Dynamic programming. Order of complexity $TN^2$. Give examples, and compare with expression above. Explain forward algo with colors. Mention that: The neat thing with NIMBLE is that it provides marginalised models through nimbleEcology, we'll get back to it in Reference forward to section below (forward algorithm). 
...
```{r}
my.constants <- list(N = nrow(y_pooled), T = 5, S = y_pooled[,'S'])
my.constants

my.data <- list(y = y_pooled[,-1] + 1)
