...
In this third chapter, you will learn the basics on Markov models and how to fit them to longitudinal data using NIMBLE. Because in real life, individuals may go undetected and their status be unknow, you will also learn how to manipulate the extension of Markov models to hidden states, so-called hidden Markov models. **Mention marginalization and pooling. **
...
In this set up, we consider a single winter. But for many species, we'll need to collect data on the long term to get a representative estimate of survival. Therefore what if we had say big $T$ five winters?
...
Let's think of a model for these data. The objective remains the same, estimating survival. To build this model, we'll make assumptions, go through its components and write down its likelihood.

**On a déjà vu chaîne Markov chapitre MCMC**
...
A realization of this Markov process is, for example:
...
Going from a winter to the next is driven by survival and mortality processes. The probability of going from alive or 1 to alive or 1 is $\phi$. Then from alive 1 to dead 2 is $1 - \phi$. And the probability to remain dead is 1, that is to go from state 2 dead to state 2 for dead.

### Transition matrix

You might have figured it out already, the core of a Markov process is made of transition probabilities between states alive and dead. 
...
All individuals are marked and released in the first winter, hence alive when first captured, which means that they are all in state alive or 1 for sure. Therefore we have $\mathbf{\delta} = \left(1, 0\right)$.
...
Before implementing this model in NIMBLE, we note that the likelihood is a product of conditional probabilities of binary events (alive or dead) that are drawn in Bernoulli distributions, or for more generality, their extension categorical distributions. **SMOOTH AND EXPLAIN CATEGORICAL DISTRIBUTIONS. MENTION ITS MULTINOMIAL WITH ONE TRIAL**.
...
Initial state is categorical with probability $\delta$. That is you have a dice with two faces, a coin, and you have some probability to be alive, and one minus that probability to be dead. Of course, it you want your Markov chain to start, you'd better say it's alive so that delta is just $(1,0)$:

\begin{align*}
   z_1 &\sim \text{Categorical}(\delta) &\text{[likelihood, }t = 1 \text{]}\\
\end{align*}

Now the main part is the dynamic of the states. Our state at $t$ depends only on your state at $t-1$, and it is a categorical random variable. And the probabilities are given by the rows of the transition matrix:
...
We also need a prior on survival. As usual we take a uniform distribution between 0 and 1, or a Beta distribution with parameters 1 and 1. Overall our model is: 
...
If $z$ at $t-1$ is alive, it is the first row in $\mathbf{\Gamma}$, 
...
that is $\color{blue}{\gamma_{z_{t-1} = 1,z_{t}} = (\phi, 1-\phi)}$.

Otherwise, if $z$ at $t-1$ is dead that is 2, then it is the second row in $\mathbf{\Gamma}$:
...
If dead you remain dead with probability equals to 1, $\color{blue}{\gamma_{z_{t-1} = 2,z_{t}} = (0, 1)}$.

## Nimble implementation

**Introduce loops. First time we see them in the book. Maybe for a single individual first, then embed time loop in individual loop. **

In Nimble, we will use the categorical distribution `dcat()`. Nimble code:
```{r}
...
Vector $\delta$ is used as a placeholder for more complex models to come. Here, you could write `z[i,1] <- 1`.
...
**Need to comment on constants, and backward reference to NIMBLE chapter and relevant section. **
...
Posterior mean and median are close to $0.8$. Cool! The data was simulated, with (true) survival $\phi = 0.8$.
...
**Note also that dcat everywhere could be replaced by dbern. Different sampler? Dcat cool because allows more than two outcomes, forward reference to next chapters.**
