...
How to implement in NIMBLE the Markov model we just built? First things first, we need some bricks that are relatively easy to put in place. Let's start with the prior on survival, the vector of initial state probabilities and the transition matrix:
```{r eval = FALSE}
...
Now we're ready to resume our standard NIMBLE workflow. First we read in data. Because we have loops and indices that do not change, we use constants as explained in Section \@ref{start-nimble}:
```{r}
...
Compare with previous table. Some 1's for alive have become 0's for non-detection, other 1's for alive have remained 1's for detection. This table $y$ is what we observe in real life. I hope to have convinced that to make the connection between observations, the $y$, and true states, the $z$,  we need to describe how observations are made (or emitted in the HMM terminology) from the states.
...
\node [state,fill=lightgray!75] (4) [left = 20mm of 5] {$z_{1}$};
...
\node [state,fill=white] (14) [above = 20mm of 4] {$y_{1}$};
...
States $z$ are in gray. Observations $y$ are in white. All individuals are first captured in the first winter $t = 1$, and are therefore all alive $z_1 = 1$ and detected $y_1 = 2$. A hidden Markov model is just two time series running in parallel. One for the states with the Markovian property, and the other of for the observations generated from the states.

An example is as follows, in which an individual is detected at first sampling occasion, detected again, then missed for the rest of the study. While on occasion $t=3$ that individual was alive $z_3=1$ and went undetected $y_3=1$, on occasions $t=4$ and $t=5$ it went undetected $y_4=y_5=1$ because it was dead $z_4=z_5=2$.
...
The likelihood is the probability of the observed data $y$ given the parameters of our model, the unkowns. Here the unknows are the parameters $\phi$, $p$ and the latent states $z$. 

Using the formula of total probability, then the likelihood of a Markov chain, we get that the likelihood of an HMM for a single individual $i$:
...
\Pr(\mathbf{y}_i) &= \Pr(y_{i,1}, y_{i,2}, \ldots, y_{i,T})\\
                ...
                &= \left(\prod_{t=1}^T{\omega_{z_{i,t}, y_{i,t}}}\right) \left(\Pr(z_{i,1}) \prod_{t=2}^T{\gamma_{z_{i,t-1},z_{i,t}}}\right)\\
\end{align*}

To obtain the likelihood of the whole dataset, we need to multiply this individual likelihood for each animal $\prod_{i=1}^N{\Pr(\mathbf{y}_i)}$. When several individuals have the same contribution, calculating their individual contribution only once can greatly reduce the computational burden, as illustrated in Section \@ref{pooled-likelihood}. 

The Bayesian approach with MCMC methods allows treating the latent states $z_{i,t}$ as if they were parameters, and to be estimated as such. However, the likelihood is rather complex with a large number of latent states $z_{i,t}$, which comes at computational costs and with slow mixing. There are situations where the latent states are the focus of ecological inference and need to be estimated (see suggestions of Further reading below). However, if not needed, you might want to get rid of the latent states and focus on the parameters, which often speeds up computations and improves mixing as shown in Section \@ref{marginalization}. Actually, you can even estimate the latent states afterwards, as illustrated in Section \@ref{decoding}.
...
It has an observation layer for the $y$'s, conditional on the $z$'s. We also consider uniform priors for the detection and survival probabilities.
...
Shall we estimate the latent states $z_{i,t}$ or not? We can get rid of the states, so that likelihood is a function of $\phi$ and $p$ only. This is the function we would maximize in a Frequentist approach. 

The neat thing with Nimble is that it provides marginalised models through nimbleEcology, we'll get back to it in Reference forward to section below (forward algorithm). 

\begin{align*}
\Pr(\mathbf{y}_i) &= \Pr(y_{i,1}, y_{i,2}, \ldots, y_{i,T})\\
                &= \sum_{z_{i,1}} \cdots \sum_{z_{i,T}} \Pr(y_{i,1}, y_{i,2}, \ldots, y_{i,T} | z_{i,1}, z_{i,2}, \ldots, z_{i,T}) \Pr(z_{i,1}, z_{i,2}, \ldots, z_{i,T})\\
                &= \sum_{z_{i,1}} \cdots \sum_{z_{i,T}} \left(\prod_{t=1}^T{\omega_{z_{i,t}, y_{i,t}}}\right) \left(\Pr(z_{i,1}) \prod_{t=2}^T{\gamma_{z_{i,t-1},z_{i,t}}}\right)\\
\end{align*}
...
**Forward algorithm.  I need to transpose the observation matrix in the NIMBLE code, this needs to be said when explaining the forward algorithm.**
...
Mention NimbleEcology.

**Compute number of nodes. Show that the z are no longer in there.**

## Pooled encounter histories {#pooled-likelihood}

In this section, we're gonna use NIMBLE functions to express the likelihood using pooled encounter histories. We use a vector `size` that contains the number of individuals with a particular encounter history. Courtesy of Chloé Nater. Implement of Turek et al. 
...
```{r}
assign('dHMMweighted', dHMMweighted, .GlobalEnv)
assign('rHMMweighted', rHMMweighted, .GlobalEnv)
```


```{r}
dHMMweighted(x = c(2,2,1,1,1),
             probInit = delta,
             probObs = Gamma,
             probTrans = Omega,
             len = 5,
...
hmm.survival <- nimbleCode({
  ...
  omega[2,2] <- 0        # Pr(dead t -> detected t)
  for (i in 1:N){
    y[i,1:T] ~ dHMMweighted(probInit = delta[1:2], 
                    probObs = omega[1:2,1:2], # observation matrix
                    probTrans = gamma[1:2,1:2], # transition matrix
                    len = T,
                    size = S[i]) # nb of sampling occasions
  }
})
```
...
```{r}
my.constants <- list(N = nrow(y_weighted), T = 5, S = y_weighted[,'S'])
my.constants

my.data <- list(y = y_weighted[,-1] + 1)
...
+ Ask Perry what he had in mind.

On peut estimer, ou bien décoder avec Viterbi, voir la différence dans https://luisdamiano.github.io/BayesHMM/articles/introduction.html. Le plus simple, après avoir expliqué Viterbi, serait d'utiliser une fonction déjà existante, celle-ci https://rdrr.io/bioc/STAN/man/getViterbi.html ou celle-là https://github.com/oliviergimenez/code_livre_variables_cachees/blob/master/code/viterbi_phispdelta.R du package HMM. Seul problème est de gérer première occasion capture où détection certaine et individu en vie. Le plus malin est peut-être de recode https://mc-stan.org/docs/2_24/stan-users-guide/hmms-section.html à la toute fin, dernière section. Reste alors à décider si on calcule viterbi à chaque itération, puis on faire la moyenne/médiane, ou bien si on prend les moyennes/médianes a posterior des paramètres et qu'on fait Viterbi https://discourse.mc-stan.org/t/how-to-get-the-best-state-sequence-using-viterbi-algorithm/18969. 
...
More about so-called marginalisation in [Yackulic et al. (2020)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1002/eap.2112). Reference forward to section below (forward algorithm). 
