...
Let's test the `getViterbi()` function with an example. In our simulated dataset, animal #15 has the encounter history (`r y[15,] + 1`) which was generated from the sequence of states (`r z[15,]`) with survival probability $\phi = 0.8$ and detection probability $p = 0.6$. Applying our function to that animal encounter history, we get:
```{r}
