...
Third step is to tell NIMBLE which parameters in your model you would like to keep track of, in other words the parameters you are interested in. In our simple model there is a single parameter `theta` for survival so you do not have much of a choice:
```{r}
parameters.to.save <- c("theta")
```

In general you will have many parameters in your model, including some of no focal interest, and having full control on verbosity will prove handy.
...
NIMBLE goes through several steps that we will explain later on. **Reference forward. Say a few words on the messages we receive. Comment on arguments of the function: E.g. ProgressBar can be supressed if too depressing.** 
...
We have `r dim(mcmc.output$chain1)[1]` samples from the posterior distribution of `theta`, which corresponds to `n.iter - n.burnin`. From there, you may want to compute the posterior mean of `theta`:
```{r}
mean(mcmc.output$chain1)
```

You may also obtain the 95% credible interval for `theta`:
```{r}
quantile(mcmc.output$chain1, probs = c(2.5, 97.5)/100)
```
...
You may use a caterpillar plot to visualise the posterior distributions of `theta` with `MCMCplot()`:
```{r}
MCMCplot(mcmc.output, 
         params = 'theta')
```
...
You may also add the diagnostics of convergence we discussed in the previous chapter:
```{r}
MCMCtrace(mcmc.output,
          pdf = FALSE,
          ind = TRUE,
...
Life expectancy directly monitored. But you can also calculate it outside NIMBLE by hand: **problem is that we show how to inspect mcmc.output below, not before, how to fix that? show the whole workflow first, then show these features.**
```{r}
