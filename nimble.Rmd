...
First you write the model code, data and initial values as you did before:
```{r}
model <- nimbleCode({
  # likelihood
  survived ~ dbinom(theta, released)
...
First we create the model as an R object:
```{r}
...
#c.mcmc <- compileNimble(mcmc, project = survival.model)
```
...
By default one chain, but can be changed. 
