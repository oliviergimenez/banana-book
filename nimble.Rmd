...
**Do I need to show how NIMBLE can simulate data; do I need to introduce MCMC efficiency?**
...
In the call to `parLapply`, we specify `X = c(2022, 666)` to ensure reproducibility where 2022 and 666 are the values we use for seed in the `workflow()` function. Note that we also have a line `set.seed(123)` in the `workflow()` function because initial values are drawn randomly. 
...
### Incomplete initialization, NA or NaN messages and simulations

When you run `nimbleMCMC()` or `nimbleModel()`, you may get warnings thrown at you by NIMBLE like 'This model is not fully initialized' or 'value is NA
or NaN even after trying to calculate'. NIMBLE tells you that this is not necessarily an error but 'reflects missing values in model variables' or incomplete initialization. NIMBLE tries to initialize nodes with NAs by drawing from priors, and it will work or not. When possible, I try to initialize all nodes or full initialization. The process can be a headache, but it helps understanding your model structure. Going back to our animal survival example, we purposedly forget to provide an initial value for `theta`:
```{r}
model <- nimbleCode({
  # likelihood
  survived ~ dbinom(theta, released)
...
Vectorization is the process of rewriting a loop so that instead of processing a single value at a time, you process a set of values at once in a vector. As an example, instead of writing:
```{r eval = FALSE}
for(i in 1:n){ 
  x[i] <- mu + epsilon[i] 
}
