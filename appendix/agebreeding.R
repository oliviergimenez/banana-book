...
dolphins <- read_csv2(here::here("dat", "dolphins.csv"), col_names = TRUE)
y <- dolphins %>%
  as.matrix()
...
viterbi_path <- function(obs, delta, gamma, omega) {
  Tlen <- length(obs)
  S <- nrow(gamma)
  # tiny floor to avoid -Inf in logs for init only (doesn't touch the model)
  eps <- 1e-12
...
generate_inits_viterbi <- function(Y, first, par){
  ...
  for(i in 1:N){
    t0 <- first[i]
    obs_i <- Y[i, t0:K]           # values in 1..4
    # Viterbi on the segment t0:K
...
# 
# # sanity checks
# rowSums(model$gamma[,])  # all ~1
# rowSums(model$omega[,])  # all ~1
# model$calculate()
# c(model$logProb_z, model$logProb_y)  # should both be finite
...
save(out, file = "agebreeding.RData")
