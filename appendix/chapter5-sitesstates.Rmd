...
Idem in nimbleEcology

```{r}
# load packages
library(nimbleEcology) # NIMBLE 
library(tidyverse) # manipulate/visualize data
library(MCMCvis) # post-process MCMC outputs
# read in data
geese <- read_csv("geese.csv", col_names = TRUE)
...
#save(mcmc.multisite.marginalized, file = "twositemarginalized.Rdata")
```
...
multisite <- nimbleCode({
  ...
  psi1[1:3] ~ ddirch(alpha[1:3])
  psi2[1:3] ~ ddirch(alpha[1:3])
  psi3[1:3] ~ ddirch(alpha[1:3])
...
mcmc.multisite <- nimbleMCMC(code = multisite, 
                             ...
                             nchains = n.chains)
# numerical summaries
#save(mcmc.multisite, file = 'geese_3sites_dirichlet.RData')
MCMCsummary(mcmc.multisite, round = 2)
```
...
multisite <- nimbleCode({
  ...
  for (s in 1:3){
    lpsi1[s] ~ dgamma(alpha[s], 1) # Y1, Y2, Y3 for psiAA, psiAB, psiAC
    psi1[s] <- lpsi1[s]/V1 # psiAA, psiAB, psiAC
    lpsi2[s] ~ dgamma(alpha[s], 1) # Y'1, Y'2, Y'3 for psiBA, psiBB, psiBC
    psi2[s] <- lpsi2[s]/V2 # psiBA, psiBB, psiCC
    lpsi3[s] ~ dgamma(alpha[s], 1) # Y''1, Y''2, Y''3 for psiCA, psiCB, psiCC
    psi3[s] <- lpsi3[s]/V3 # psiCA, psiCB, psiCC
  }
  V1 <- sum(lpsiA[1:3])
  V2 <- sum(lpsiB[1:3])
  V3 <- sum(lpsiC[1:3])
...
zinits <- y
zinits[zinits==0] <- sample(c(1,2,3), sum(zinits==0), replace = TRUE)
initial.values <- function(){list(beta1 = rnorm(3, 0, 1), 
                                  beta2 = rnorm(3, 0, 1), 
                                  beta3 = rnorm(3, 0, 1), 
                                  p1 = runif(1, 0, 1), 
                                  p2 = runif(1, 0, 1), 
...
save(mcmc.multisite, file = "geese_3sites_logit.RData")
```
...
```{r}
png(file="multistate_local_minimav2_Page_07.png", width = 6, height = 5, units = 'in', res = 300)
plot(x = grid, y = dev, type = 'l', xlab = expression(psi^{21}), ylab = 'deviance', col = 'black', lwd = 3, cex.lab = 1.2)
...
# numerical summaries
MCMCsummary(mcmc.multisite, round = 2)
save(mcmc.multisite, file = "localminima.RData")
