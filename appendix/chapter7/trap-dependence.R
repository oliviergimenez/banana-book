...
# trap-dependence with calonectris
...
hmm.td <- nimbleCode({
  ...
  for (i in 1:N){
    for (t in first[i]:(K-1)){
      ...
      gamma[1,1,i,t] <- phi[i,t] * pprime          # Pr(alive 1 t -> alive 1 t+1)
      gamma[1,2,i,t] <- phi[i,t] * (1 - pprime)    # Pr(alive 1 t -> alive 2 t+1) // no transition
      gamma[1,3,i,t] <- 1 - phi[i,t]               # Pr(alive t -> dead t+1)
      gamma[2,1,i,t] <- phi[i,t] * p               # Pr(alive 1 t -> alive 1 t+1) // no transition
      gamma[2,2,i,t] <- phi[i,t] * (1 - p)         # Pr(alive 1 t -> alive 2 t+1)
      gamma[2,3,i,t] <- 1 - phi[i,t]               # Pr(alive t -> dead t+1)
      gamma[3,1,i,t] <- 0                     # Pr(dead t -> alive 1 t+1)
      gamma[3,2,i,t] <- 0                     # Pr(dead t -> alive 1 t+1)
  ...
  omega[2,1] <- 1                     # Pr(alive state 2 t -> non-detected t)
  omega[2,2] <- 0                     # Pr(alive state 2 t -> detected t)
  ...
  omega.init[2,1] <- 1                # Pr(alive state 2 t -> non-detected t)
  omega.init[2,2] <- 0                # Pr(alive state 2 t -> detected t)
...
create_initial_z <- function(y, first, phi = 0.9, p = 0.5, pprime = 0.5, pi = 1) {
  ...
  # Observation matrix: y ∈ {1 = not seen, 2 = seen}
...
save(out, file = "calonectris-w-trap-dep.RData")
