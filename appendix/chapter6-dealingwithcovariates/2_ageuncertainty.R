...
# read in data
y_raw <- as.matrix(read.table("apennine-bear.txt"))
# encounter histories
...
hmm.age <- nimbleCode({
  ...
  # their initial state in the model, thus providing the model with a subset 
  # of individuals of known age
  for (i in 1:N){
  ...
  omega.init[1,1] <- 0                      
  omega.init[1,2] <- betaCCC                
  omega.init[1,3] <- betaCCA                
  omega.init[1,4] <- 1 - betaCCC - betaCCA  
  omega.init[2,1] <- 0                      
  omega.init[2,2] <- betaACC                
  omega.init[2,3] <- betaACA                
  omega.init[2,4] <- 1 - betaACC - betaACA  
  omega.init[3,1] <- 1                      
  omega.init[3,2] <- 0                      
  omega.init[3,3] <- 0                      
  omega.init[3,4] <- 0                      
...
# (could/should have used Dirichlet priors or multinomial logit link instead)
