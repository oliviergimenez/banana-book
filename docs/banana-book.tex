...
\DocumentationTok{\#\# 0.2320 0.4612}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{end\_time }\OtherTok{\textless{}{-}} \FunctionTok{Sys.time}\NormalTok{()}
\NormalTok{end\_time }\SpecialCharTok{{-}}\NormalTok{ start\_time}
\DocumentationTok{\#\# Time difference of 23.1 secs}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{end\_time }\OtherTok{\textless{}{-}} \FunctionTok{Sys.time}\NormalTok{()}
\NormalTok{end\_time }\SpecialCharTok{{-}}\NormalTok{ start\_time}
\DocumentationTok{\#\# Time difference of 26.73 secs}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{end\_time }\OtherTok{\textless{}{-}} \FunctionTok{Sys.time}\NormalTok{()}
\NormalTok{end\_time }\SpecialCharTok{{-}}\NormalTok{ start\_time}
\DocumentationTok{\#\# Time difference of 23.53 secs}
\FunctionTok{MCMCsummary}\NormalTok{(mcmc.output, }\AttributeTok{round =} \DecValTok{2}\NormalTok{)}
\DocumentationTok{\#\#     mean   sd 2.5\%  50\% 97.5\% Rhat n.eff}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-220-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-240-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-256-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-267-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-268-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-294-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-298-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-310-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-336-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-339-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-340-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-353-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-367-1.pdf}}
...
\caption{\label{tab:unnamed-chunk-376}Comparison with Pradel et al. (2007) Table 5, Model B (no genetically sexed anchors). Our error rates are 1 − x[i]. Pradel's m4 is time-varying; the estimates provided by the main author for m4(t) were for t=1..10: 0.001, 0.002, 0.004, 0.007, 0.014, 0.026, 0.047, 0.084, 0.146, 0.242.}
\centering
...
Gray wolf is a social species with hierarchy in packs which may reflect in demography. Shirley Pledger in a series of papers developed heterogeneity models in which individuals are assigned in two or more classes with class-specific survival/detection probabilities. \citet{cubaynes_importance_2010} used HMMs to account for heterogeneity in the detection process due to social status, see also \citet{pradel2009} w/ example on black-headed gull. Dominant individuals tend to use path more often than others, and these paths are where we look for scats.

You may consider more classes, and select among models, see \citet{cubaynes2012}, not an easy problem. Alternatively, you may also go for a non-parametric approach and let the data tell you how many classes you need. This is relatively easy to do in NIMBLE, see \citet{turek_bayesian_2021}. More about individual heterogeneity in \citet{gimenez2018ih}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# generate heterogeneous detection probabilities}
\NormalTok{phi }\OtherTok{\textless{}{-}} \FloatTok{0.7} \CommentTok{\# survival}
\NormalTok{prop\_class1 }\OtherTok{\textless{}{-}} \FloatTok{0.2} \CommentTok{\# pi}
\NormalTok{p\_class1 }\OtherTok{\textless{}{-}} \FloatTok{0.8} \CommentTok{\# low prop are highly detectable}
\NormalTok{p\_class2 }\OtherTok{\textless{}{-}} \FloatTok{0.3}
\NormalTok{nind }\OtherTok{\textless{}{-}} \DecValTok{400} \CommentTok{\# nb of ind}
\NormalTok{nyear }\OtherTok{\textless{}{-}} \DecValTok{10} \CommentTok{\# duration of the study}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{MCMCsummary}\NormalTok{(mcmc.phipmix, }\AttributeTok{round =} \DecValTok{2}\NormalTok{)}
\DocumentationTok{\#\#     mean   sd 2.5\%  50\% 97.5\% Rhat n.eff}
\DocumentationTok{\#\# phi 0.70 0.01 0.67 0.70  0.73 1.00  1074}
\DocumentationTok{\#\# pi  0.70 0.03 0.63 0.70  0.76 1.01   722}
\DocumentationTok{\#\# pp1 0.45 0.02 0.40 0.45  0.49 1.00   492}
\DocumentationTok{\#\# pp2 0.49 0.03 0.43 0.48  0.55 1.01   632}
\end{Highlighting}
\end{Shaded}
...
The usual formulation creates a serious limitation: individuals are not allowed to switch between groups over time (e.g., from class 1 to class 2), because the transition matrix does not permit it. In other words, any individual seen \textgreater{} 1 time (it's detected after the first observation occasion) can \emph{never} change group assignments away from their initial value group assignment. Why this formulation fails? The problem is subtle but fundamental: in a standard HMM, transitions between states are governed by a Markov process. If the transition matrix says an individual in class 1 today must stay in class 1 (or die), then the individual is permanently locked in that class. As a result, once an individual is assigned to a group through its initial latent state (e.g., class 1), it can never change class during sampling. Even worse, any individual seen on multiple occasions (say, years 1 and 4) must stay alive during those years, the latent state cannot ``jump'' between classes without violating the transition constraints, therefore, the initial class assignment is fixed forever, and the sampler cannot explore the alternative class, no matter how much data supports it.

There are several solutions

\begin{itemize}
\item
  Marginalize over the latent states: Instead of sampling the latent states \(z\), you can marginalize them out, which avoids the group-switching problem entirely. This is the principle behind the \texttt{nimbleEcology} package.
\item
  Use a group-level latent variable: Rather than coding groups as HMM states, define a latent group assignment (e.g., \texttt{g{[}i{]}\ \textasciitilde{}\ dcat(pi{[}1:2{]})}) for each individual. The HMM then conditions on that fixed group, allowing for proper inference and switching during MCMC.
\item
  Custom block sampler: Design a custom MCMC sampler that updates the entire latent history of an individual (\texttt{z{[}i,\ 1:K{]}}) at once. This allows for group-switching in a consistent way, though it's more advanced and computationally heavier.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{MCMCsummary}\NormalTok{(mcmc.phipmix.marginalized, }\AttributeTok{round =} \DecValTok{2}\NormalTok{)}
\DocumentationTok{\#\#     mean   sd 2.5\%  50\% 97.5\% Rhat n.eff}
\DocumentationTok{\#\# phi 0.72 0.02 0.69 0.72  0.75    1  1120}
\DocumentationTok{\#\# pi  0.72 0.08 0.54 0.73  0.87    1   298}
\DocumentationTok{\#\# pp1 0.27 0.05 0.18 0.27  0.35    1   343}
\DocumentationTok{\#\# pp2 0.79 0.07 0.65 0.79  0.92    1   336}
\end{Highlighting}
\end{Shaded}
...
Multievent formulation à la \citet{pradeltrapdep2012}. Also add example w/ individual time-varying covariate. Application w/ data from Ana' paper in 2011.
...
\begin{verbatim}
##        mean   sd 2.5%  50% 97.5% Rhat n.eff
## p      0.46 0.06 0.36 0.46  0.57 1.03    87
## phi1   0.75 0.02 0.71 0.75  0.80 1.00   405
## phi2   0.87 0.02 0.84 0.87  0.90 1.08   128
## pi     1.00 0.00 0.99 1.00  1.00 1.00   371
## pprime 0.79 0.02 0.75 0.79  0.82 1.01   238
\end{verbatim}
...
Can be generalized to sites, states, see \citet{pradeltrapdep2012}.
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-387-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-391-1.pdf}}

Stopover duration

\pandocbounded{\includegraphics[keepaspectratio]{banana-book_files/figure-latex/unnamed-chunk-392-1.pdf}}
