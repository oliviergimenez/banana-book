...
The acceptance probability is high `r mean(accept)` and would need some tuning (try increasing the default standard deviation of 0.2). 
...
for (t in 2:steps){ # repeat steps 2-4 (step 5)
  ...
  X <- runif(1, 0, 1) # spin continuous spinner
  if (X < R){
    theta.post2[t] <- theta_star
  }
  else{
    theta.post2[t] <- theta.post[t-1]
  }
}
...
simul.bgr <- function(steps, inits){
  ...
  for (i in 1:nb.replicates){
    ...
    for (t in 2:steps){ # repeat steps 2-4 (step 5)
      ...
      X <- runif(1, 0, 1) # spin continuous spinner
      if (X < R){
        theta.post[i,t] <- theta_star
      }
      else{
        theta.post[i,t] <- theta.post[i,t-1]
      }
    }
  }
...
As expected, `n.eff` is less than the number of MCMC iterations because of autocorrelation. Only when the standard deviation of the proposal distribution is 0.2 and mixing is good (Figures \@ref(fig:tracechainlength) and \@ref(fig:acfchainlength)) we get a satisfying effective sample size. 
