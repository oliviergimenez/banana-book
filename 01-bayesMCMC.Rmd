...
This likelihood can be visualised easily: 
...
(ref:captionbeta) The distribution beta($a$,$b$) for different values of $a$ and $b$. Note that for $a = b = 1$, we get the uniform distribution between 0 and 1, see top left panel.
...
You may check that the mean we calculate empirically matches the expectation of a beta distribution^[If $X$ is a random variable with distribution $\text{beta}(a, b)$, then $E(X) = \displaystyle{\frac{a}{a + b}}$]:
```{r}
mean(sample_from_posterior) # compute mean with Monte Carlo integration
20/(20+39) # expectation of beta(20,39)
```

Another useful numerical summary is the credible interval within which our parameter falls with some probability, usually 0.95 hence a 95$\%$ credible interval. Finding the bounds of a credible interval requires calculating quantiles, which in turn involves integrals and the use of Monte Carlo integration. A 95$\%$ credible for winter survival can be obtained in `R` with:
```{r}
quantile(sample_from_posterior, probs = c(2.5/100, 97.5/100))
```

Now what is a Markov chain? A Markov chain is a random sequence of numbers, in which each number depends only on the previous number. An example is the weather in my home town in Southern France, in which a sunny day is most likely to be followed by another sunny day, say with probability 0.8, and a rainy day is rarely followed by another rainy day, say with probability 0.1. The dynamic of this Markov chain is captured by the transition matrix $\mathbf{\Gamma}$:
$$
\begin{matrix}
& \\
\mathbf{\Gamma} = 
...
Each row of the transition matrix converges to the same distribution $(0.82, 0.18)$ as the number of steps increases. Convergence happens no matter which state you start in, and you always have probability 0.82 of the day being sunny and 0.18 of the day being rainy. Back to MCMC, the trick is that you can build a Markov chain with a given stationary distribution set to be the desired posterior distribution. 

Putting Monte Carlo and Markov chains together, MCMC allows us to generate a sample of values whose distribution converges to the posterior distribution (Markov chain), and we can use this sample of values to calculate any posterior summaries (Monte Carlo), such as posterior means and credible intervals. This summarizes the core spirit of MCMC algorithms. 
 
There are several ways of constructing these Markov chains^[You might have heard about the Metropolis-Hastings or the Gibbs sampler. Have a look to <https://github.com/chi-feng/mcmc-demo> for an interactive gallery of MCMC algorithms.]. Here I will illustrate the Metropolis algorithm and how to implement it in practice^[This presentation is largely inspired by @alberthu2019].
...
Now we're ready for steps 2, 3 and 4. Actually, we will write a look to take care of step 5 as well. Remember we start at initial value 0.5 and run the algorithm for $100$ steps or iterations. 
```{r}
...
Visually, you may look at the chain in Figure \@ref(fig:chain).
```{r chain, echo = FALSE, fig.align='center', fig.cap = "Visualisation of a Markov chain starting at value 0.5, with steps or iterations on the x-axis, and generated values on the y-axis. This graphical representation is called a traceplot."}
...
```{r twochains, echo = FALSE, fig.align='center', fig.cap = "Traceplot of survival for two chains starting at 0.2 (yellow) and 0.5 (blue) run for 100 steps."}
...
Notice that we do not get the exact same results. The question is to know whether we have reached the stationary distribution. Let's increase the number of steps and run a chain with 1000 iterations as in Figure \@ref(fig:longchain).
```{r longchain, echo = FALSE, fig.align='center', fig.cap = "Traceplot of survival for a chains starting at 0.5 and 1000 steps."}
# set up the scene
steps <- 1000
...
This is what we're after, a traceplot that looks like a beautiful lawn, more in the next section.  I find it informative to look at the animated version of Figure \@ref(fig:longchain), it helps understanding the stochastic behavior of the algorithm, and also to realise how the chains converge to their stationary distribution, see Figure \@ref(fig:animlongchain).
...
```
```{r animlongchain, echo = FALSE, out.width="100%", fig.align='center', fig.cap = "Animated traceplot of survival with three chains starting at 0.2, 0.5 and 0.7 run for 1000 steps."}
...
d <-
  d %>% 
  mutate(proposal_sd = str_c("Proposal SD (away) = ", proposal_sd),
...
trace <- d %>% 
  ...
  theme_light(base_size = 14)
library(forecast)
plot1 <- ggAcf(x = d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 0.02"]) + 
  theme_light(base_size = 14) + ggtitle("Proposal SD (away) = 0.02")
plot2 <- ggAcf(x = d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 0.2"]) + 
  theme_light(base_size = 14) + ggtitle("Proposal SD (away) = 0.2")
plot3 <- ggAcf(x = d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 2"]) + 
  theme_light(base_size = 14) + ggtitle("Proposal SD (away) = 2")
```
...
```{r neff, echo = FALSE}
neff1 <- coda::effectiveSize(d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 0.02"])
neff2 <- coda::effectiveSize(d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 0.2"])
neff3 <- coda::effectiveSize(d$accepted_traj[d$proposal_sd=="Proposal SD (away) = 2"])
df <- data.frame("Proposal SD (away)" = c(0.02, 0.2, 2),
                 "n.eff" = c(neff1, neff2, neff3))
...
+ **We emphasise that the beauty of MCMC is that the updating procedure remains relatively simple, no matter how complex the posterior distribution of interest.**
