...
**utiliser des références un peu plus (critique des bouquins de Kéry et Schaub), mais pas trop quand même**
...
When we use a binomial likelihood together with a uniform prior, the posterior distribution has an explicit form that we can calculate by hand^[**faire qqch sur conjugacy?**]. In Figure \@ref(fig:compar), We superimpose the exact posterior and its numerical approximation to realise that the two distributions are indistinguishable, suggestion that the numerical approximation is more than fine. 
...
**parler de réplicats d'une chaine de Markov plutôt que de plusieurs Markov chains?**
...
The idea of Markov chain Monte Carlo (MCMC) is to simulate values from a Markov chain which has a stationary distribution equal to the posterior distribution you're after. In practice, you run a Markov chain multiple times starting from over-dispersed initial values. You discard iterations in an initial burn-in phase and achieve convergence when all replicates reach the same regime. From there, you run the chain long enough and proceed with calculating Monte Carlo estimates (numerical summaries) for model parameters. 
