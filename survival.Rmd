...
Mention relative fit (WAIC) vs absolute fit. Put this section next to the WAIC section. 

Statistical models rely on assumptions, and the CJS model makes no exception. 

In the Bayesian framework, posterior predictive checks. Explain principle. However omnibus, test all directions departure from good fit of the CJS to data. Also need to figure out distance, and summary statistics, not easy w/ HMM. 

Good think is that we have tests. 
...
Example on CJS w/ Dipper. We will see ways to account for transience and trap-dependence. Gof for this model obtained easily by sustracting. I am aware that I'm mixing Bayesian and Frequentist frameworks, information theoretic and null hypothesis testing, and some colleagues might not like it, but best of the two worlds, and just dismiss the amount of research done on gof testing?!

To close this section on model assumptions, I should stress out that survival refers to the study area, and so we need to think carefully about what survival does actually mean in capture-recapture. More precisely, mortality and permanent emigration are confounded. Therefore we estimate apparent survival, not true survival. Apparent survival probability is the product of true survival and study area fidelity. Consequently, apparent survival is always lower than true survival unless study area fidelity is exactly 1. Use caution with interpretation.
...
Or perform Test.3Sr for transience Test3.Sm:
```{r}
test3sr <- test3sr(dip.fem.hist, dip.fem.freq)
test3sr
```

Or Test2.Ct for trap-dep
```{r}
test2ct <- test2ct(dip.fem.hist, dip.fem.freq)
test2ct
```

What to do if these tests are significant? If you detect a transient effect, then 2 age classes should be considered on the survival probability to account for this issue (fast forward covariate section below). If trap dependence is significant, Cooch and White (2017) illustrate how to use a time-varying individual covariate to account for this effect (fast forward section on covariate below), and Pradel and Sanz (2012) recommend multievent models (fast forward case study).
