...
```{r marking, fig.ncol = 2, echo = FALSE, fig.cap = "Animal individual marking. Top-left: rings; Top-right: ear-tags; Bottom left: coat patterns; Bottom right: ADN feces", fig.show = "hold", fig.subcap = c("ring", "ear-tag", "coat patters", "ADN feces"), out.width="50%"}
...
**Wondering whether I should demonstrate marginalization here, and pooled encounter histories.**
...
Conveniently, NIMBLE calculates WAIC for you. The only modifications we need to make are i) to monitor all nodes, including the latent states and ii) to add `WAIC = TRUE` in the call to the `nimbleMCMC()` function. For example, for the CJS model, we write:
```{r eval = FALSE}
...
Lower values of WAIC imply higher predictive accuracy, thefore we would favor model with constant parameters. **Add WAIC weights.**
...
We use nested indexing here when specifying survival in the transition matrix. E.g. for year $t = 2$, `phi[flood[t]]` gives `phi[flood[2]]` which will be `phi[2]` because `flood[2]` is 2 that is a flood year.
...
Having a look to the numerical summaries, we see that as expected, survival in flood years (`phi[2]`) is much lower than survival in non-flood years (`phi[1]`):
```{r, echo = FALSE}
...
Survival between 1982 and 1983 (`phi[2]`) was greatly affected and much lower than on average. This decrease corresponds to the high water flow in 1983 and the flood. These results are in line with our previous findings obtained by considering a covariate for nonflood vs. flood years.
...
**Show how to use values of the covariate de-standardized.** 

**Mention interaction bw sex and wing length w/ logit(phi[i]) <- beta[1] + beta[2] * sex[i] + beta[3] * winglength[i] + beta[4] * sex[i] * winglength[i] and explain.**
...
The prior on the standard deviation of the random effect is uniform between 0 and 10. **Explain how to pick prior for SD.**
...
Here we used `equals(t, first[i])` which renders 1 when $t$ is the first encounter and 0 otherwise. Therefore we distinguish survival of the first interval after first encounter $\phi_1$ (`logit(phi[i,t]) <- beta[1] + beta[2]`) from survival afterwards $\phi_{1+}$ (`logit(phi[i,t]) <- beta[1]`).
...
+ For model comparison, the WAIC can be used to evaluate the relative predictive performance of capture-recapture models. **Refer to case study on RJMCMC.**
...
+ [Buckland (2016)](https://research-repository.st-andrews.ac.uk/handle/10023/8869) provides a historical perspective on the CJS model and anecdotes. The monography by [Lebreton et al. (1992)](https://esajournals.onlinelibrary.wiley.com/doi/10.2307/2937171) is a must-read to better understand the CJS model and its applications. The HMM formulation of the CJS model was proposed by [Gimenez et al. (2007)](https://oliviergimenez.github.io/pubs/Gimenezetal2007EcologicalModelling.pdf) and [Royle (2008)](https://onlinelibrary.wiley.com/doi/10.1111/j.1541-0420.2007.00891.x).

+ [Gimenez et al. (2009)](https://oliviergimenez.github.io/pubs/Gimenezetal2009-weakidentifiability.pdf) deals with parameter redundandy in capture-recapture models in a Bayesian framework. For an exhaustive treatment, see [Cole's excellent book](https://www.routledge.com/Parameter-Redundancy-and-Identifiability/Cole/p/book/9781498720878).

+ Relative to model comparison, I warmly recommend [McElreath's book](https://www.routledge.com/Statistical-Rethinking-A-Bayesian-Course-with-Examples-in-R-and-STAN/McElreath/p/book/9780367139919) to better understand WAIC (there is a video as well, see <https://www.youtube.com/watch?v=vSjL2Zc-gEQ>). The paper by [Gelman et al. 2014](https://link.springer.com/article/10.1007/s11222-013-9416-2) is also much helpful. 

+ On posterior predictive checks, you may check out [Conn et al. 2018](https://doi.org/10.1002/ecm.1314). The `R2ucare` package is introduced in [Gimenez et al. 2018](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.13014).

+ Temporal heterogeneity is addressed in papers by [Grosbois et al. 2008](https://oliviergimenez.github.io/pubs/Grosboisetal2008.pdf) and [Frederiksen et al. 2014](https://oliviergimenez.github.io/pubs/Frederiksenetal2013JAE.pdf), while individual heterogeneity is reviewed by [Gimenez et al. 2018](https://oliviergimenez.github.io/pubs/GimenezCamGaillard2017Oikos.pdf).
