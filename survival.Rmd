...
Conveniently, NIMBLE calculates WAIC for you. The only modifications we need to make are i) to monitor the latent states and ii) to add `WAIC = TRUE` in the call to the `nimbleMCMC()` function. For example, for the CJS model, we would write:
```{r eval = FALSE}
...
data.frame(model = c("model with both survival and detection constant",
                     "model with time-dependent survival, constant detection",
                     "model with constant survival, time-dependent detection",
                     "CJS model"),
